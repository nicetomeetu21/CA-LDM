...
class Encoder(nn.Module):
    def __init__(self, z_channels, n_hiddens, downsample, image_channel=3, norm_type='group', padding_type='replicate', num_groups=32):
        super().__init__()
        ...
        self.final_block = nn.Sequential(
            ...
            SiLU(),
            torch.nn.Conv3d(out_channels,z_channels,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)
        )
...
class Decoder(nn.Module):
    def __init__(self, z_channels,n_hiddens, upsample, image_channel, norm_type='group', num_groups=32):
        super().__init__()
        ...
        self.final_block = nn.Sequential(
            torch.nn.Conv3d(z_channels, in_channels,
                            kernel_size=3,
                            stride=1,
                            padding=1),
...
class DecoderSR_old(nn.Module):
    def __init__(self, in_channels, upsample, image_channel, norm_type='group', num_groups=32):
        super().__init__()
...
class DecoderSR_old_v2(nn.Module):
    def __init__(self, in_channels, upsample, norm_type='group', num_groups=32):
        super().__init__()
