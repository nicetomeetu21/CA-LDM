...
class Decoder(nn.Module):
    def __init__(self, n_hiddens, upsample, image_channel, norm_type='group', num_groups=32):
        ...
        for i in range(max_us):
            ...
            print(in_channels, out_channels, num_groups)
            block.up = Upsample(
                in_channels, out_channels, with_conv=True)
            block.res1 = ResBlock(
                out_channels, out_channels, norm_type=norm_type, num_groups=min(out_channels, num_groups))
            block.res2 = ResBlock(
                out_channels, out_channels, norm_type=norm_type, num_groups=min(out_channels, num_groups))
            self.conv_blocks.append(block)
            #n_times_upsample -= 1
...
class Upsample(nn.Module):
    def __init__(self, in_channels, out_channels, with_conv):
        super().__init__()
        print('using interpolation+conv upsample')
...
class SamePadConvTranspose3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=True, padding_type='replicate'):
        ...
        pad_input = sum(pad_input, tuple())
        #print(pad_input)
        #print(tuple([k - 1 for k in kernel_size]))
        self.pad_input = pad_input
        self.padding_type = padding_type
    ...
    def forward(self, x):
        ...
        out = self.convt(F.pad(x, self.pad_input, mode=self.padding_type))
